...
def GetIFS(index, historyPricesList, currentPricesList, forcastPricesList, hurst):
    ...
    for i in range(1, fNum):
        a = aParam
        e = (fNum * (i - 1)) / fNum
        c = (tempPricesList[i] - tempPricesList[i-1]) / fNum - dScale * (tempPricesList[fNum-1]-tempPricesList[0]) / fNum
...
def ReturnSchdule(intervalIndex, dataBar, bsFlag):
    ...
    if bsFlag:
        ...
        fiveMinQuantity = pow(x,4) *  fiveMinQuantity
    else:
...
if __name__ == '__main__':
    ...
    '''ifsList = GetIFS(0, HistoryAveragePricesList, CurrentAveragePricesList, ForcastAveragePricesList, 0.6)
    print("ifsList", ifsList)
    pointList = Interpolation(ifsList, 4)
    DrawTheDataForARow(pointList, 'ro')'''
