...
def ReturnSchdule(intervalIndex, dataBar, bsFlag):
    ...
    if bsFlag:
        ...
        fiveMinQuantity = pow(x,8) *  fiveMinQuantity
    else:
...
class Mystrategy(StrategyBase):
    ...
    def on_bar(self, bar):
        ...
        if IntervalBorderList.count(timeStampNow) > 0:
            ...
            OneMinVolList = ReturnSchdule(intervalIndex, bar, bool(0))
            print("OneMinVolList",intervalIndex,OneMinVolList)
        minNow = timeStampNow.minute % 5
        order = td.open_long(Market, StockId, 0, OneMinVolList[minNow])
