...
def ReturnSchdule(intervalIndex, dataBar, bsFlag):
    oneMinVolList = []
    upAdjustRatio = [0.10, 0.15, 0.20, 0.25, 0.30]
    downAdjustRatio = [0.30, 0.25, 0.20, 0.15, 0.10]
    fiveMinQuantity = CalNextLargeIntervalVol(RemindQuantity, intervalIndex)
    print("fiveMinQuantity:", intervalIndex, fiveMinQuantity)
    openPrice = dataBar.open
    lastAveragePrice = 0
    if intervalIndex > 0:
        lastAveragePrice = CurrentAveragePricesList[intervalIndex - 1]
    else:
        lastAveragePrice = HistoryAveragePricesList[len(HistoryAveragePricesList) - 1]
    if bsFlag:
        # 卖出情况
        x = openPrice / lastAveragePrice
        fiveMinQuantity = pow(x, 6) * fiveMinQuantity
    else:
        x = lastAveragePrice / openPrice
        fiveMinQuantity = pow(x, 6) * fiveMinQuantity
    forcastPrice = ForcastAveragePricesList[intervalIndex]
    if bsFlag:
        # 卖出情况
        if openPrice < forcastPrice:
            # 由小到大调整
            for i in range(0, len(upAdjustRatio)):
                tempQuantity = ReturnOneMinSchdule(i, upAdjustRatio, fiveMinQuantity)
                fiveMinQuantity -= tempQuantity
                oneMinVolList.append(tempQuantity)
        else:
            for i in range(0, len(downAdjustRatio)):
                tempQuantity = ReturnOneMinSchdule(i, downAdjustRatio, fiveMinQuantity)
                fiveMinQuantity -= tempQuantity
                oneMinVolList.append(tempQuantity)
    else:
        if openPrice < forcastPrice:
            ...
            for i in range(0, len(upAdjustRatio)):
                ...
                fiveMinQuantity -= tempQuantity
                oneMinVolList.append(tempQuantity)
    return oneMinVolList


def ReturnOneMinSchdule(index, weighList, reminedQuantity):
    sumOfRatio = 0
    for i in range(index, len(weighList)):
        sumOfRatio += weighList[i]
    currentRatio = weighList[index] / sumOfRatio
    result = ToIntShare(reminedQuantity * currentRatio)
    return result


def InitIndexToOrderIdMap():
    global IndexToOrderIdMap
    for i in range(0, 48):
        IndexToOrderIdMap[i] = []
...
class Mystrategy(StrategyBase):
    ...
    def on_bar(self, bar):
        ...
        if IntervalBorderList.count(timeStampNow) > 0:
            # 5分钟整点
            CurrentTimeIndex = IntervalBorderList.index(timeStampNow)
            print(timeStampNow)
            intervalIndex = IntervalBorderList.index(timeStampNow)
            # fiveMinQuantity = CalNextLargeIntervalVol(RemindQuantity, intervalIndex)
            # OneMinVolList = SplitLargeIntoOneMinAverage(fiveMinQuantity, 5)
            OneMinVolList = ReturnSchdule(intervalIndex, bar, bool(1))
            print("OneMinVolList", intervalIndex, OneMinVolList)
        minNow = timeStampNow.minute % 5
        order = td.open_short(Market, StockId, 0, OneMinVolList[minNow])
        OrderRecordList.append(OneMinVolList[minNow])
        if CurrentTimeIndex != -1:
            IndexToOrderIdMap[CurrentTimeIndex].append(order.cl_ord_id)
    ...
    def on_backtest_finish(self, indicator):
        for i in range(0, 48):
            ...
            totalVolume = 0
            averagePrice = 0
            orderIdList = IndexToOrderIdMap[i]
            if len(orderIdList) != 0:
                for item in orderIdList:
                    for order in OrderList:
                        if order.cl_ord_id == item:
                            totalVolume += order.filled_volume
                            totalAmount += order.filled_amount
                            break
            if totalVolume != 0:
                averagePrice = totalAmount / totalVolume
            IndexToCostMap[i] = averagePrice
        TotalFilledAveragePrice = TotalFilledAmount / TotalFilledVolume
        print("Finished! TotalFilledAveragePrice:", TotalFilledAveragePrice)
        print("OrderRecordList", OrderRecordList)
        invervalPriceList = []
        for valueItem in IndexToCostMap.values():
            invervalPriceList.append(valueItem)
        print("invervalPriceList", invervalPriceList)
    ...
    def on_order_filled(self, res):
        ...
        global TotalFilledAmount
        global TotalFilledVolume
        global OrderList
        TotalFilledAmount += res.filled_amount
        TotalFilledVolume += res.filled_volume
...
if __name__ == '__main__':
    ...
    InitIndexToOrderIdMap()
    TodayVRList = GetRatioListOfToday(Market + '.' + StockId, StartHistoryDay, EndHistoryDay, HistoryDays, WeightList)
    print("TodayVRList:", TodayVRList)
    IntervalBorderList = GetIntervalList(TradeDate)
    print("VolumeMatrix", VolumeMatrix)
    print("AmountMatrix", AmountMatrix)
    FiveMinHisAveragePriceMatrix = GetHistoryIntervalPrice(VolumeMatrix, AmountMatrix)
    print("FiveMinHisAveragePriceMatrix", FiveMinHisAveragePriceMatrix)
    ...
    CurrentAveragePricesList = ConvertBarsToAveragePricesList(currentDataList)
    HistoryAveragePricesList = ConvertMatrixToList(FiveMinHisAveragePriceMatrix)
    print("HistoryAveragePricesList", HistoryAveragePricesList)
    print("CurrentAveragePricesList", CurrentAveragePricesList)
    #ForcastAveragePricesList = GetForcastAveragePricesListByMove(HistoryAveragePricesList, CurrentAveragePricesList)
    ForcastAveragePricesList = [7.74, 7.72, 7.71, 7.70, 7.67, 7.64, 7.74, 7.65, 7.65, 7.67, 7.66, 7.67, 7.66, 7.67, 7.67, 7.68, 7.68, 7.68, 7.69, 7.70, 7.70, 7.70, 7.71, 7.70, 7.70, 7.71, 7.70, 7.70, 7.71, 7.73, 7.76, 7.79, 7.79, 7.78, 7.79, 7.81, 7.81, 7.80, 7.78, 7.77, 7.77, 7.77, 7.78, 7.78, 7.77, 7.77, 7.77, 7.75]
