...
StartHistoryDay = '2016-01-18'
EndHistoryDay = '2017-03-13'

HistoryDays = 20 # 历史数据N天
...
def FillVolumnRatioMatrix(volumnMatrix):
    ...
    for row in range(timeSpan):
        ...
        for col in range(len(volumnMatrix[row])):
            if sumOfCol == 0:
                volumnRatioMatrix[row][col] = 1 / len(volumnMatrix[row])
            else:
...
def GetWaveletHurst(historyPriceMatrix):
    ...
    coeffs = pywt.wavedec(dataList, 'db8', level=7)
    cA2, cD7, cD6, cD5, cD4, cD3, cD2, cD1 = coeffs
...
def GetWaveletHurstList(historyPriceList):
    coeffs = pywt.wavedec(historyPriceList, 'db8', level=7)
    cA2, cD7, cD6, cD5, cD4, cD3, cD2, cD1 = coeffs
...
#将价格序列转换成价格差序列
def ConvertPriceToDifference(priceList):
    differList = []
    differList.append(0)
...
if __name__ == '__main__':
    myStrategy = Mystrategy(
        ...
        strategy_id='c9d2dd48-1074-11e7-bdc3-28d2443e5dda',
    ...
    openPriceList = ConvertMatrixToList(FiveMinOpenPriceMatrix)
    openPriceDifferList = ConvertPriceToDifference(openPriceList)
    print("openPriceList", openPriceDifferList)
