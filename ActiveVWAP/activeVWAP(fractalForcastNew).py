...
def DrawDataNew(listdata, origindata, xlist,xlabelList,sign1, sign2):
    x1 = []
    y1 = []
    for item in listdata:
        x1.append(item[0])
...
#index since 0
def CreateOriginData(historydata, currentdata, forcastdata, index):
    origindata = []
    for i in range(0, len(historydata)):
        origindata.append([i, historydata[i]])
    tempIndex1 = len(origindata) - 1
    for i in range(0, index):
        origindata.append([tempIndex1 + 1 + i, currentdata[i]])
    tempIndex2 = len(origindata) - 1
    origindata.append([tempIndex2 + 1, forcastdata[index]])
    return origindata
...
if __name__ == '__main__':
    ...
    xlabelList = [1, "Predict"]
    for predictIndex in range(0, 48):
        xlist = [0, basicPointNum + predictIndex]
        ifsList = GetIFS(predictIndex, HistoryFiveMinOpenPriceList, CurrentOpenPricesList, ForcastAveragePricesList, 0.6818)
        startPoint = [0, HistoryFiveMinOpenPriceList[0]]
        pointList = Interpolation(startPoint, ifsList, 2)
        originData = CreateOriginData(HistoryFiveMinOpenPriceList, CurrentOpenPricesList, ForcastAveragePricesList, predictIndex)
        DrawDataNew(pointList, originData, xlist, xlabelList, 'r.', 'bo')
